...
[group('db')]
migrate *ARGS:
    docker compose --env-file .env.prod -f compose.yml -f cloudflared.compose.yml exec backend sh -c 'goose -dir /migrations postgres "$DATABASE_URL" up' {{ARGS}}
...
[group('docker')]
ps:
    docker compose --env-file .env.prod -f compose.yml -f cloudflared.compose.yml ps

[group('docker')]
build-prod:
    docker compose --env-file .env.prod -f compose.yml -f cloudflared.compose.yml build
